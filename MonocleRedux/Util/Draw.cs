...
namespace Monocle
{
    public static class Draw
    {
        ...
        internal static void Initialize(GraphicsDevice graphicsDevice)
        {
            ...
            DefaultFont = new BitmapFont(MTexture.FromFile(Path.Combine("Content", "acorn.png")), 8, 16);
        ...
        public static void Draw9Slice(MTexture tex, Rectangle rect, int sliceDim)
        {
            tex.Draw9Slice(rect, sliceDim);
        }

        public static void TextureFillRect(MTexture tex, Rectangle rect, Vector2 offset)
        {
            Vector2 pos = rect.Location.ToVector2();
            for (; pos.X < rect.Location.X + rect.Width; pos.X += tex.Width)
            {
                for (; pos.Y < rect.Location.Y + rect.Height; pos.Y += tex.Height)
                {
                    tex.Draw(pos);
                }
            }
